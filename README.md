...
* **Determinar posibles insights de las ventas:** Identificación de estacionalidad, preferencias del usuario, comportamiento de los métodos de pago y segmentación regional.
...
La validez de las conclusiones estratégicas se sustenta en un proceso de ETL riguroso, transformando datos transaccionales en variables de decisión con una visión de analista senior.
...
* **Métricas de Eficiencia Logística:** Cálculo del `delivery_delta` (promesa vs. realidad), una variable clave para medir la fiabilidad operativa y el cumplimiento de SLA.
...
* **Gestión de Nulos en Reseñas:** A pesar de presentar un 56% de nulos en los comentarios de texto, se decidió **preservar la variable**. Para un economista, el "silencio" o la falta de comentario es un dato en sí mismo; sin embargo, las reseñas existentes se utilizaron como el termómetro para medir el impacto financiero de los errores de stock.
...
* **Hiper-concentración Regional:** El estado de **San Pablo (SP)** domina el 37.5% de la facturación global. El clúster Sudeste (SP-RJ-MG) concentra el **62.5% de los ingresos totales**, lo que define a la eficiencia logística en este eje como el principal driver de rentabilidad.
...
El análisis revela una correlación directa y destructiva entre el incumplimiento logístico y la percepción de marca:
* **Predictores de Detracción:** El retraso en el `carrier_date` (entrega al transportista) es el principal disparador de calificaciones de 1 estrella. Las órdenes marcadas como "cancelado" o "indisponible" concentran la mayor tasa de detractores, erosionando el **Customer Lifetime Value (LTV)**.
...
* **Falta de Stock (Stockout):** Impacto negativo de **$162,821.47**. Este valor representa ventas iniciadas pero no concretadas por problemas de inventario.
